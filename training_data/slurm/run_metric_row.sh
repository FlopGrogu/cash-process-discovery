#!/usr/bin/env bash
# Generic metric job payload. Resource settings belong on the sbatch command
# or in scripts/submit_metric_manifest_slurm.sh, not in this row runner.
#SBATCH --job-name=process-metrics
...
MANIFEST_PATH="${1:-}"
[[ -n "${MANIFEST_PATH}" ]] || fail "Usage: sbatch --array=0-N slurm/run_metric_row.sh <metric-manifest.csv>"
[[ -n "${SLURM_ARRAY_TASK_ID:-}" ]] || fail "SLURM_ARRAY_TASK_ID is not set"
METRIC_MANIFEST_ROW_OFFSET="${METRIC_MANIFEST_ROW_OFFSET:-${MANIFEST_ROW_OFFSET:-0}}"
[[ "${METRIC_MANIFEST_ROW_OFFSET}" =~ ^[0-9]+$ ]] || fail "METRIC_MANIFEST_ROW_OFFSET must be a nonnegative integer"
ACTUAL_ARRAY_TASK_ID=$((METRIC_MANIFEST_ROW_OFFSET + SLURM_ARRAY_TASK_ID))
...
if hostname | grep -qi "madeira"; then
  fail "Refusing to run metrics on login node madeira. Submit this script with sbatch."
fi
...
echo "METRIC_MANIFEST_ROW_OFFSET=${METRIC_MANIFEST_ROW_OFFSET}"
echo "ACTUAL_METRIC_MANIFEST_ROW_INDEX=${ACTUAL_ARRAY_TASK_ID}"
echo "SLURM_JOB_PARTITION=${SLURM_JOB_PARTITION:-}"
echo "SLURM_JOB_NAME=${SLURM_JOB_NAME:-}"
echo "METRIC_PROFILE=${METRIC_PROFILE:-}"
...
if row_index < 0 or row_index >= len(rows):
    raise SystemExit(
        f"Actual metric manifest row index {row_index} is outside manifest range 0-{len(rows) - 1}"
    )
...
"${PYTHON_BIN}" scripts/run_metric.py \
