"""
Anitan's EXACT feature implementation, ported from origin/test-anitan commit
04d91d52 (Feature_Extension_Exploration.ipynb). The functions below are retained
so that feature values match Anitan's notebook exactly. A thin wrapper
``extract_log_features()`` runs his pipeline (his own XES parser -> case features ->
compute_extended_features_for_log) on a single .xes/.xes.gz file.

Do not "clean up" this file: the point is byte-for-byte fidelity with Anitan.
"""
...
def load_xes(path: Path, max_traces: int | None = None) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    if "time:timestamp" in events.columns:
        events["time:timestamp"] = pd.to_datetime(
            events["time:timestamp"], utc=True, errors="coerce"
        )
...
def summarize_log(
    log_name: str, path: Path, events: pd.DataFrame, case_features: pd.DataFrame
) -> dict:
    summary = {
        "log": log_name,
    ...
    if "event_count_derived" in case_features.columns and not case_features.empty:
        summary["events_per_case_mean"] = round(
            float(case_features["event_count_derived"].mean()), 3
        )
...
def count_length_two_loop_patterns(trace: tuple[str, ...]) -> int:
    return sum(
        1 for i in range(len(trace) - 2) if trace[i] == trace[i + 2] and trace[i] != trace[i + 1]
    )
...
def compute_extended_features_for_log(log_name: str, frames: dict[str, pd.DataFrame]) -> dict:
    ...
    trace_tuples = [
        split_variant(value)
        for value in case_features.get("variant_tuple", variant_series).dropna()
    ]
    ...
    start_counts = (
        case_features["first_activity"].value_counts(dropna=True)
        if "first_activity" in case_features
        else pd.Series(dtype=float)
    )
    end_counts = (
        case_features["last_activity"].value_counts(dropna=True)
        if "last_activity" in case_features
        else pd.Series(dtype=float)
    )
    activity_counts = (
        events["concept:name"].value_counts(dropna=True)
        if "concept:name" in events
        else pd.Series(dtype=float)
    )
    ...
    row["rs4pd_repetitions_intra_trace_avg"] = (
        sum(repeated_activity_count(t) for t in trace_tuples) / len(trace_tuples)
        if trace_tuples
        else None
    )
    ...
    pair_counts = (
        pairs.value_counts(["source", "target"]).reset_index(name="count")
        if not pairs.empty
        else pd.DataFrame(columns=["source", "target", "count"])
    )
    if pair_counts.empty or n_activities == 0:
        ...
        observed_pairs = {
            (item.source, item.target) for item in pair_counts.itertuples(index=False)
        }
        bidirectional_pairs = {
            tuple(sorted((source, target)))
            for source, target in observed_pairs
            if source != target and (target, source) in observed_pairs
        }
        unordered_possible_pairs = n_activities * (n_activities - 1) / 2
        row["rs4pd_flow_concurrency_ratio"] = (
            len(bidirectional_pairs) / unordered_possible_pairs if unordered_possible_pairs else 0
        )
        row["rs4pd_flow_density"] = (
            len(observed_pairs) / (n_activities * n_activities) if n_activities else None
        )
        row["rs4pd_length_one_loops_count"] = int(
            (pair_counts["source"] == pair_counts["target"]).sum()
        )
    ...
    if not variant_counts.empty:
        ...
        for pct in [1, 5, 10, 20, 50, 75]:
            ...
            row[f"ratio_top_{pct}_variants"] = (
                sum(occurrences[:cutoff]) / n_traces if n_traces else None
            )
    ...
    if not activity_counts.empty:
        ...
        row["activities_iqr"] = safe_percentile(activity_counts, 0.75) - safe_percentile(
            activity_counts, 0.25
        )
        row["activity_entropy_custom"] = entropy_from_counts(activity_counts)
        row["most_common_activity_share_custom"] = (
            float(activity_counts.max() / n_events) if n_events else None
        )
        row["rare_activity_ratio_custom"] = (
            float((activity_counts <= 1).sum() / len(activity_counts))
            if len(activity_counts)
            else None
        )
    ...
    if not start_counts.empty:
        ...
        row["most_common_start_share_custom"] = (
            float(start_counts.max() / n_traces) if n_traces else None
        )
    ...
    if not end_counts.empty:
        ...
        row["most_common_end_share_custom"] = (
            float(end_counts.max() / n_traces) if n_traces else None
        )
    ...
    if not pair_counts.empty:
        ...
        row["top_dfg_edge_share_custom"] = float(
            pair_counts["count"].max() / pair_counts["count"].sum()
        )
        row["rare_dfg_edge_ratio_custom"] = float(
            (pair_counts["count"] <= 1).sum() / len(pair_counts)
        )
        self_loop_activities = set(
            pair_counts.loc[pair_counts["source"] == pair_counts["target"], "source"]
        )
        row["length_one_loops"] = len(self_loop_activities) / n_activities if n_activities else 0
    else:
        row["length_one_loops"] = None

    row["relative_number_of_traces_with_repetition"] = (
        sum(has_non_self_loop_repetition(t) for t in trace_tuples) / len(trace_tuples)
        if trace_tuples
        else None
    )
    row["avg_event_repetition_intra_trace"] = (
        sum(repeated_activity_count(t) for t in trace_tuples) / len(trace_tuples)
        if trace_tuples
        else None
    )
...
# ---------------------------------------------------------------------------
# Wrapper: run Anitan's exact pipeline on one log file.
# ---------------------------------------------------------------------------


def extract_log_features(xes_path, max_traces: int | None = None) -> dict:
    """Return Anitan's exact feature dict for a single XES (.xes/.xes.gz) file."""
    path = resolve_portable_path(xes_path)
...
FEATURE_NAMES = ANITAN_FEATURE_NAMES


def extract_features_from_xes(xes_path: str) -> dict:
    """Anitan's exact features for a XES (.xes/.xes.gz) file as a {name: value} dict.

    Missing keys (rare edge cases in his function) are filled with NaN so the
