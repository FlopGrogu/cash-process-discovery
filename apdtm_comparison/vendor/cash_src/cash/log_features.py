"""
Extended CASH log-feature implementation.

The functions below parse a XES/XES.GZ event log, derive case-level structures,
and compute the fixed feature set used by the CASH dataset. A thin wrapper
``extract_log_features()`` runs the full pipeline on a single log file.
"""
...
# ---------------------------------------------------------------------------
# Wrapper: run the feature pipeline on one log file.
# ---------------------------------------------------------------------------

def extract_log_features(xes_path, max_traces: int | None = None) -> dict:
    """Return the extended feature dict for a single XES (.xes/.xes.gz) file."""
    path = Path(xes_path)
